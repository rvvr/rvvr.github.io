<template>
  <dialog class="modal" ref="modalsRoomClosed">
...
export default {
  computed: {
    ...mapState(useRoomStore, ['roomRating', 'round_status', 'current_round_number', 'max_round_number']),
    ...mapState(useUserStore, ['user']),
  },

  watch: {
    round_status(round_status) {
      if (this.current_round_number === this.max_round_number && round_status === 'closed') {
        GameSocket.stop()

        setTimeout(() => {
          if (this.$refs.modalsRoomClosed) this.$refs.modalsRoomClosed.showModal()
        }, 1000)
      }
    },
  },
